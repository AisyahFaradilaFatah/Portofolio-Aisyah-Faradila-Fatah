# Hi there, I'm Aisyah Faradila Fatah! 👋
**IoT Engineer | Data Analyst | Web Developer**

Mahasiswa semester 5 dengan spesialisasi pada Artificial Intelligence (AI) dan Mekatronika. Memiliki keahlian teknis dalam pemrograman, pengembangan perangkat lunak, serta analisis data. Berfokus pada pemanfaatan data kompleks untuk menciptakan solusi inovatif yang mendukung pengambilan keputusan strategis.

### 🔭 Current Focus
* Mengembangkan sistem *Precision Agriculture* menggunakan **Deep Learning**.
* Eksplorasi algoritma **Machine Learning** untuk data kesehatan (Healthcare).

### 🛠️ Tech Stack
* **Languages:** Python, C++, PHP, SQL
* **Data & AI:** Pandas, Scikit-Learn, OpenCV, Fuzzy Logic
* **IoT & Hardware:** ESP32, Arduino, Sensors
* **Web:** HTML/CSS, MySQL

### 📂 Featured Projects
| Project | Tech Stack | Deskripsi |
| :--- | :--- | :--- |
| **Smart Oryza Sativa Monitor** | Python, ESP32 | Monitoring nutrisi padi real-time dengan ML, Sistem Pakar & IoT. |
| **BPJS Predictive Model** | Python, Pandas | Prediksi Lama Jeda Tindakan dari dataset skala besar. |
| **Fire Fighter Robot** | Thonny, ESP32 | Robot pemadam api otonom dengan PID Controller. |
| **Yamaha Dealer Platform** | Laragon, Figma | Web katalog motor interaktif. |

---
[LinkedIn](https://www.linkedin.com/in/aisyahfaradilafatah/) • [Email](mailto:aisyahfatah1@gmail.com)
